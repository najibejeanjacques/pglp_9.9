...
public class CercleDAO implements CustomDAO<Cercle> {
	...
	public int insertCustomer(Cercle t) {
		...
		try (Connection connect = derby.createConnection()) {
			PreparedStatement prepare = connect.prepareStatement(
					"INSERT INTO Cercle (nom, rayon, coord1, coord2)" +
					"VALUES (?, ?, ?, ?)");
	...
	public void updateCustomer(Cercle t) {
		...
		try (Connection connect = derby.createConnection()) {
			PreparedStatement prepare = connect.prepareStatement(
					"UPDATE Cercle SET coord1 = ?, coord2 = ? where nom = ?");
	...
	public void findAllCustomer() {
		...
		try (Connection connect = derby.createConnection()) {
			PreparedStatement prepare = connect.prepareStatement(
					"SELECT id, nom, rayon, coord1, coord2 FROM Cercle ");
			...
			while(result.next())
			{
				do
				{
					...
					System.out.println(colonne2+" = Cercle(("+colonne4+","+colonne5+"), "+colonne3+")");
				}
				while(result.next());
			}
		}
	...
	public Cercle finfByName(String name) {
		...
		try (Connection connect = derby.createConnection()) {
			PreparedStatement prepare = connect.prepareStatement(
					"SELECT id, nom, rayon, coord1, coord2 FROM Cercle where nom = ?");
